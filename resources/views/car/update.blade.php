        ...
        <div class="col-12 col-md-6 col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('car.update', $model->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="region_id" class="form-control">
                                        ...
                                        @foreach($regionList as $key => $value)
                                            <option value="{{ $key }}"
                                                    @if($key == $model->region_id) selected @endif>{{ $value }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="user_id[]" class="form-control select2" multiple>
                                        @foreach($userList as $id => $name)
                                            <option value="{{ $id }}"
                                                    @if(in_array($id, $model->getUsers())) selected @endif>{{ $name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
